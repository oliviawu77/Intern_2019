	  	...
	  	//				 3. 使用者不存在(導到錯誤頁面，顯示使用者不存在訊息)  	
	  	 if( Pwd_valid != null )
	   	{
	  		if(Pwd_valid.equals(pwd)) //情況 1
	  		{
	  			...
	  			String wel_message = "Welcome, " + sex_msg + name; //產生個別使用者的歡迎訊息
	  			request.setAttribute("wel_message",wel_message); //request.setAttribute(指定要傳給的對象名字，傳入的參數)
	  			String message = db.getMsg(name);
