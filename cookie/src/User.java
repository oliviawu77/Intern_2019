...
public class User extends HttpServlet {
    ...
    private Optional<Cookie> userCookie(Cookie[] cookies) {
        return Stream.of(cookies)
                     .filter(cookie -> "user".equals(cookie.getName()) && ("Olivia".equals(cookie.getValue())||"caterpillar".equals(cookie.getValue())))
                     .findFirst();
    }
    ...
    private void userHtml(HttpServletRequest request, HttpServletResponse response)
        ...
        out.println("</head>");
        out.println("<body>");
        out.println("<h1>" + request.getAttribute("user") + "¤wµn¤J</h1>");
        out.println("</body>");
        out.println("</html>");
    }
} 